...
RUN apt-get update && apt-get install -y --no-install-recommends \
    apt-utils git curl vim nano unzip openssh-client wget \
    build-essential cmake \
    libopenblas-dev

RUN apt-get install -y --no-install-recommends python3.5 python3.5-dev \
    python3-pip python3-tk libjpeg-dev zlib1g-dev && \
...
RUN cd /usr/local/src/caffe && mkdir build && cd build && \
    cmake -D python_version=3 -D BLAS=open -D USE_OPENCV=ON .. && \
    make -j"$(nproc)" all && \
    make install
